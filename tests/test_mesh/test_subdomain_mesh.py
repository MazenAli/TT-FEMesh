...
from ttfemesh.quadrature.quadrature import GaussLegendre2D, QuadratureRule, QuadratureRule2D
...
class TestSubdomainMesh:
    def test_abstract_base_class_cannot_be_instantiated(self):
        ...
        with pytest.raises(TypeError):
            SubdomainMesh(
                subdomain=mock_subdomain, quadrature_rule=mock_quadrature_rule, mesh_size_exponent=2
            )
    ...
    def test_initialization_with_valid_parameters(self):
        class TestSubdomainMesh(SubdomainMesh):
            ...
            def _validate_ref_coords(self, *_, **__):
                pass
...
def subdomain_mesh():
    ...
    return SubdomainMesh2D(subdomain=quad, quadrature_rule=qrule, mesh_size_exponent=3)
...
def quad_mesh():
    ...
    quad = QuadFactory.create(p1, p2, p3, p4)
    qrule = GaussLegendre2D(2)
    return QuadMesh(quad=quad, quadrature_rule=qrule, mesh_size_exponent=3)
...
class TestSubdomainMesh2D:
    ...
    def test_ref2domain_map_quad(self, subdomain_mesh):
        ...
        expected_corners = np.array([[0, 0], [2, 0], [3, 1], [-0.5, 3]])
        ...
        midpoints = np.array([[0, -1], [1, 0], [0, 1], [-1, 0]])
        expected_midpoints = np.array([[1, 0], [2.5, 0.5], [1.25, 2], [-0.25, 1.5]])
        ...
        center = np.array([[0, 0]])
        expected_center = np.array([[1.125, 1.0]])
    ...
    def test_ref2element_map_quad(self, subdomain_mesh):
        ...
        expected_corners = np.array(
            [
                ...
                [2.0 / 7.0, 0],
                [
                    -0.5 / 7.0 + 1.0 / 7.0 * (2.0 + 1.0 / 7.0 + 0.5 / 7.0),
                    1.0 / 7.0 * (3.0 - 2.0 / 7.0),
                ],
                [-0.5 / 7.0, 3.0 / 7.0],
            ]
    ...
    def test_ref2element_jacobian_quad(self, subdomain_mesh):
        ...
        def func(x):
            return subdomain_mesh.ref2element_map(index, x)
                ...
                for i in range(2):
                    for j in range(2):
                        ...
                        with warnings.catch_warnings():
                            ...
                            approx = mesh.ref2element_jacobian(
                                index, mesh.quadrature_rule.get_points_weights()[0][q : q + 1]
                            )[0, i, j]
                        delta += np.abs(exact - approx)
    ...
    def test_get_jacobian_invdets_quad(self, mesh_fixture, request):
        ...
        for qindex in product((0, 3), repeat=mesh.mesh_size_exponent):
            ...
            for q in range(num_quadrature_points):
                ...
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore", UserWarning)
                    approx = 1.0 / np.linalg.det(jacobian_tensors[index][q])
                delta += np.abs(exact - approx)
    ...
    def test_cross_func_multiple_points(self, subdomain_mesh):
        ...
        with pytest.raises(ValueError, match="Only one evaluation point is supported for TCA"):
...
class TestQuadMesh:
    def test_initialization(self):
        ...
        quad_mesh = QuadMesh(
            quad=mock_quad, quadrature_rule=mock_quadrature_rule, mesh_size_exponent=2
        )
    ...
    def test_linear_interpolation(self):
        ...
        with patch(
            "ttfemesh.mesh.subdomain_mesh.interpolate_linear2d", return_value=mock_tt_interpolant
        ):
